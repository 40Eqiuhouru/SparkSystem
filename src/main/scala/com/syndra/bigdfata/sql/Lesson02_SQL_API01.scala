...
object Lesson02_SQL_API01 {
  def main(args: Array[String]): Unit = {
    ...
    dataDF.createTempView("word_count")

    val df: DataFrame = session.sql("SELECT * from word_count")
    df.show()
    df.printSchema()

    println("—————————— V1 ——————————")

    session.sql("select word, count(*) from (select explode(split(line, ' ')) as word from word_count) as v1 group by v1.word").show()

    println("—————————— V2 ——————————")

    // 面向 API 时, df 相当于 from tab
    ...
    *  */
    ...
    res.write.mode(SaveMode.Append).parquet("D:\\ideaProject\\bigdata\\bigdata-spark\\data\\out\\word_count")

    println("—————————— V2.2 ——————————")

    val frame: DataFrame = session.read.parquet("D:\\ideaProject\\bigdata\\bigdata-spark\\data\\out\\word_count")
    frame.show()
    frame.printSchema()
